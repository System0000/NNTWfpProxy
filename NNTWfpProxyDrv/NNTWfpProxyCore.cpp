...
)
{
	...
	//关于三环：进行下面关于WSAIoctl的时候&bytesReturned这参数是必须要传的，否则内核会拒绝处理
	//在三环进行TCP连接的时候,会导致被当前回调函数无限重定向,需要在三环通过WSAIoctl记录重定向记录,通过这个判断是否已经记录(被重定向过),如果是的话就不进行重定向
	...
	//你可以这样使用WSAIoctl,,redirectRecords是一个缓冲区指针，我不知道他是什么，不填这个参数貌似无法使用，所以我为他分配了4096字节的缓冲区
	...
	pNNTWfpContext pNNTWfpContextBuffer = (pNNTWfpContext)ExAllocatePool(NonPagedPool, sizeof(NNTWfpContext));//申请的缓冲区貌似不用释放他，因为好像wfp内部会释放他
														  /*It seems that the applied buffer does not need to be released, because it seems that wfp will release it internally.*/
	...
	ConnectRequest->localRedirectContextSize = sizeof(NNTWfpContext);//把原始ip端口信息放在一个缓冲区并转成重定向上下文(win7以上才有的东西)，然后在客户端使用
									...
									//获取重定向上下文，也就是存放原始ip信息的缓冲区，读取原始ip转发到目标代理服务器*/
	...
	ConnectRequest->localRedirectTargetPID = 0xFFFF;//这个貌似可以不用按标准来写，因为我填充成0xFFFF在三环照样能获取到重定向上下文
